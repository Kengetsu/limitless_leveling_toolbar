...
var URL_ROOT = "https://limitless-leveling.com/";
...
function goAction(ev) {
  //Check key used and do labeled function.
  var key = ev.code;
  switch (true) {
...
var timer = {
  travel: function () {
    //Add delay when traveling across the map.
    setTimeout(function () {
      traveling = false;
    }, 60);
  },
  train: function (trainType) {
    //Handle alerting user for when training is complete.
    ...
    if (seal.checked) {
      //Type is short - 0, long - 1, extended - 2.
      times = [
        baseTrainLength,
        baseTrainLength * 4 * 1.5,
        Math.round(baseTrainLength * 30 * 1.5),
      ];
    } else {
      times = [baseTrainLength, baseTrainLength * 4, baseTrainLength * 30];
    }

    var delay = boost.checked
      ? times[trainType] - times[trainType] * clanBoost
      : times[trainType];
...
var missions = {
  gen: function () {
    //Generates mission list based on what rank is selected.
    ...
    for (let i = 0; i < rank; i++) {
      missionsAvailable.push({
        name: missionRanks[i] + " Rank",
        id: "start_mission=1&rank=" + (i + 1),
      });
    }
...
var enemy = {
  gen: function () {
    // Generate list of enemy based on rank selected.
  ...
  arena: function () {
    //Start fight based on seleced enemy.
...
function arenaJutsu(array) {
  // Using jutsu information from cookie array, fill out form information.
...
function addJutsu(type, name, value, weapon) {
  //Append jutsu to array
  if (jutsuList.length == MAX_JUTSU_SLOTS) {
    // Prompt to clear jutsu if array is holding ${MAX_JUTSU_SLOTS} jutsu.
  ...
  if (jutsuList.length < MAX_JUTSU_SLOTS) {
    ...
    Cookies.set("jutsu", JSON.stringify(jutsuList), {
      expires: 365,
      path: "/",
      secure: true,
      sameSite: "None",
...
function consumeRamen(selection) {
  // Eat tier of ramen.
...
function remapKey(ev) {
  ...
  if (key in keyMap[set]) {
    var accept = confirm(
      `Are you sure you want to map ${set} : ${action} to ${newKey}`,
    );
    ...
    if (keyConflict) {
      if (
        !confirm(
          `Key is already bound to ${keyConflict[0]} : ${keyConflict[2]}, are you sure you want to clear the binding for ${keyConflict[2]}`,
        )
      ) {
        return false;
      }
    ...
    alert(
      "There is something wrong with your key mapping, please try the restore default button then try again",
    );
    return;
  }
  Cookies.set("customKeys", JSON.stringify(keyMap), {
    expires: 365,
    path: "/",
    secure: true,
    sameSite: "None",
...
function clearBinding(keySet, key, action, reload = false) {
  if (keySet == undefined || key == undefined || action == undefined)
    return false;
...
function train(evt) {
  $("#trainingType").prop(
    "name",
    $("#trainingType").find(":selected").data("type"),
  );
...
function trainChange(evt) {
  ...
  Cookies.set("previousSelections", JSON.stringify(previousSelections), {
    expires: 365,
    path: "/",
    secure: true,
    sameSite: "None",
...
function selectionChange(evt) {
  ...
  if (element.id == "selectMission") {
    let missionType = document
      .querySelector("#" + element.id + " option:checked")
...
function validateKeyMapping(currentMap) {
  ...
  for (let set in defaultMap) {
    for (let key in defaultMap[set]) {
      ...
      if (Object.values(currentMap[set]).includes(defaultMap[set][key]))
        continue;
      if (
        currentMap[set]["unmapped"] != undefined &&
        currentMap[set]["unmapped"].indexOf(defaultMap[set][key]) != -1
      )
        continue;
  ...
  if (changedKeys.length > 0) {
    alert(
      `Your key mapping has updated. The following keys were changed ${changedKeys.join(",")}`,
    );
    Cookies.set("customKeys", JSON.stringify(newMap), {
      expires: 365,
      path: "/",
      secure: true,
      sameSite: "None",
...
function clearTrainingJutsu(all = false) {
  ...
  if (all) {
    ...
    Cookies.set("trainingJutsu", JSON.stringify(trainingJutsuList), {
      expires: 365,
      path: "/",
      secure: true,
      sameSite: "None",
...
function setTrainJutsu() {
  ...
  if (trainingJutsuList.length == MAX_JUTSU_SLOTS) {
    // Prompt to clear jutsu if array is holding 6 jutsu.
    var accept = prompt("Max jutsu reached, clear list?", "yes/no");
    accept = accept.toUpperCase();
    if (accept == "YES") {
